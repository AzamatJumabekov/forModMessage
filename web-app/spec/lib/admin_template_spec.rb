RSpec.describe AdminTemplate do
  ...
  it 'Удаляет шаблон SMS_OTP_RU, метод delete' do
    ...
    template.delete
    expect(AdminTemplate.templates_list.sort).to eq(["EMAIL_OTP_RU", "SMS_OTP_EN"].sort)
  end
  ...
  it 'Сохраняет изменения шаблона(название шаблона с SMS_OTP_RU на SMS_OTP_KZ), метод update' do
    ...
    edited_file.update
    expect(AdminTemplate.templates_list.sort).to eq(["EMAIL_OTP_RU", "SMS_OTP_KZ", "SMS_OTP_EN"].sort)
  end
  ...
  it 'Создаёт и просматривает новый шаблон SMS_OTP_KZ, метод create' do
    ...
    AdminTemplate.new(payload_kz).create
